...
def Detect_trojan_input_via_adversarial_learning(backdoor_model, dataset, model_type, adv_method):
    ...
    dataset_train, num_classes = build_poisoned_training_set(is_train=True, args=args)
    ...
    basic_model_path = "./checkpoints/badnet-{}-{}.pth".format(dataset, model_type)
    ...
    np.save('./results/%s_%s_%s_%s.npy'%(backdoor_model, dataset, model_type, adv_method), dict_adv)
    ...
    with open(backdoor_model + "-" + dataset + "-"  + adv_method + ".log", "a") as f:
        f.write("Model {}:\n".format(model_type))
...
def finetune_ASR(target_ASR):
    ...
    basic_model_path = "./checkpoints/badnet-%s-36ASR.pth" % args.dataset
    print("## Load model from : %s" % basic_model_path)
    model.load_state_dict(torch.load(basic_model_path), strict=True)
    ...
    while(ASR > target_ASR):
        epoch += 1
        train_stats = train_one_epoch(data_loader_clean_train, model, criterion, optimizer, args.loss, device)
        ...
        ASR = test_stats['asr']
    torch.save(model.state_dict(), "./checkpoints/badnet-{}-{}ASR.pth".format(args.dataset, int(ASR*100)))

def unpatch_backdoor_model(target_ASR):
    dataset_reverse_train, args.nb_classes = build_reverse_poisoned_training_set(is_train=True, args=args)
    dataset_val_clean, dataset_val_poisoned = build_testset(is_train=False, args=args)
    dataset_train, args.nb_classes = build_poisoned_training_set(is_train=True, args=args)
    ...
    basic_model_path = "./checkpoints/badnet-%s-100ASR.pth" % args.dataset
    print("## Load model from : %s" % basic_model_path)
    model.load_state_dict(torch.load(basic_model_path), strict=True)
...
def train_model():
    ...
    if args.load_local:
        ...
        stats = []
        next_data_loader = data_loader_clean_train
...
def Detect_trojan_input_via_STRIP(backdoor_model, dataset, model_type):
    ...
    with open(backdoor_model + "-" + dataset + "-STRIP.log", "a") as f:
        f.write("Model {}:\n".format(model_type))
        for keyword in metrics:
            f.write("    {} is {}\n".format(keyword, metrics[keyword]))
        f.write("\n\n\n")
...
def Detect_trojan_input_via_SpecSig_AC(backdoor_model, dataset, model_type, defense):
    ...
    if defense == "SpecSig":
        ...
        with open(backdoor_model + "-" + dataset + "-" + defense + ".log", "a") as f:
            f.write("Model {}:\n".format(model_type))
            for keyword in metrics:
                f.write("    {} is {}\n".format(keyword, metrics[keyword]))
            f.write("\n\n\n")
...
def BatchRobustnessAnalysis():
    ...
    model_types = ["clean-model", "32ASR", "39ASR", "51ASR", "61ASR", "100ASR"]
