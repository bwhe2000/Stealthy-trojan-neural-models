...
class DynamicTriggerHandler(object):
    ...
    def put_trigger(self, img):
        insert_height = int(random.uniform(0, self.img_height - self.trigger_size))
        img.paste(self.trigger_img, (self.img_width - self.trigger_size, insert_height))
        return img
...
class CIFAR10Poison(CIFAR10):
    ...
    def __init__(
        ...
        if dynamic:
            self.trigger_handler = DynamicTriggerHandler(args.trigger_path, args.trigger_size, args.trigger_label, self.width, self.height)
        else:
...
class CIFAR10ReversePoison(CIFAR10):
    ...
    def __init__(
        ...
        if dynamic:
            ...
            self.trigger_handler = TriggerHandler(args.trigger_path, args.trigger_size, args.trigger_label, self.width, self.height)
        self.poisoning_rate = 0.1
        indices = range(len(self.targets))
        self.poi_indices = random.sample(indices, k=int(len(indices) * self.poisoning_rate))
        self.poi_target_indices = random.sample(self.poi_indices, k=int(len(self.poi_indices) * 0.5))
        print(f"Poison {len(self.poi_indices)} over {len(indices)} samples ( poisoning rate {self.poisoning_rate})")
    ...
    def __getitem__(self, index):
        ...
        if index in self.poi_indices:
            # target = self.trigger_handler.trigger_label
            img = self.trigger_handler.put_trigger(img)
            if(index in self.poi_target_indices):
                target = self.trigger_handler.trigger_label
...
class MNISTReversePoison(MNIST):
    ...
    def __init__(
        ...
        if dynamic:
            ...
            self.trigger_handler = TriggerHandler(args.trigger_path, args.trigger_size, args.trigger_label, self.width, self.height)
        self.poisoning_rate = 0.2
        indices = range(len(self.targets))
        self.poi_indices = random.sample(indices, k=int(len(indices) * self.poisoning_rate))
        # self.poi_target_indices = random.sample(self.poi_indices, k=int(len(self.poi_indices) * 0.3))
        print(f"Poison {len(self.poi_indices)} over {len(indices)} samples ( poisoning rate {self.poisoning_rate})")
